...
#A groove can't go all the way through
class WikiHouse::GrooveConnector < WikiHouse::Connector
  attr_reader :rows

  def initialize(depth_in_t: 1, length_in_t: nil, count: 1, width_in_t: nil, thickness: nil, rows: 1)
    super(length_in_t: length_in_t, count: count, width_in_t: width_in_t, thickness: thickness)
    @rows = rows
  end

  def draw_groove!(location: nil)
